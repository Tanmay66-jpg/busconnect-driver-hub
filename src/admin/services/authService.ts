...
export const authService = {
    ...
    signup: async (name: string, email: string, password: string): Promise<{ success: boolean, error?: string }> => {
        try {
            ...
            // Sync profile to new Supabase tables
            await supabaseService.createUserProfile(user.uid, name, email, 'email');
    ...
    verifyPhoneOtp: async (otp: string, name?: string): Promise<{ success: boolean, error?: string }> => {
        try {
            ...
            // Sync profile to new Supabase tables
            await supabaseService.createUserProfile(firebaseUser.uid, userData.name, userData.phoneOrEmail, 'phone');
    ...
    logout: async () => {
        await signOut(auth);
        localStorage.removeItem(CURRENT_USER_KEY); localStorage.removeItem("user_role");
    },
